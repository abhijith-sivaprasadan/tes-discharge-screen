...
| [P0.3](docs/RESULTS.md#p03-is-scalar-soc-a-sufficient-packed-bed-state) | Done | Scalar SOC is **not** a sufficient state: near-term deliverable power scatters up to 220% at matched SOC across *hand-constructed* bed states. Reported, not hidden. |
| [P0.3.1](docs/RESULTS.md#p031-does-the-reduced-state-survive-realistic-cycling) | Done | Under *realistic* charge/discharge cycling (the same solver run forward and backward, not hand-built states), deviation from the discharge-only curve stays under **4.3%** -- within P0.3's own 5% tolerance, unlike the hand-constructed worst case. |
...
| [Phase C3](docs/RESULTS.md#phase-c3-full-technology-ranking-matrix) | Done | Full technology-ranking matrix (packed bed / molten salt / PCM x 2 temperatures x 3 load profiles, 15 paired cases, 30 solves), now pricing packed bed's own blower parasitic load: **the SOC-dependent correction never changes which technology is cheapest, and packed bed still wins even after correctly charging it for an operating cost the other two technologies don't have modelled.** |
| [C3.1](docs/RESULTS.md#c31-does-the-ranking-survive-a-corrected-pcm-capex-citation) | Done | A primary-source audit found PCM's storage-CAPEX citation was for the wrong material class (low-temperature building PCM, not high-temperature industrial). Re-run at the corrected 15,000-21,000 EUR/MWh-th literature range: **PCM still never beats packed bed** -- the wrong citation inflated the cost gap ~6x but did not change the ranking. |
...
| [P6.3](docs/RESULTS.md#p63-heat-exchanger-quality-sensitivity) | Done | Every other result assumes a **perfect** heat exchanger (0 C approach). A realistic 30 C approach temperature drops the "correction starts to matter" threshold from theta_req=0.9 down to **0.5**, and produces the same zero-capacity cliff P6 found -- from a worse heat exchanger alone, nothing else changed. |
