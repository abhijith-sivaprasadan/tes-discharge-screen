...
**A follow-up audit pass (this session) re-searched the parameters that most
directly drive the technology-ranking result -- CAPEX energy/power components,
lifetime -- specifically to check the earlier search-quoted citations, not just
add new ones.** Direct PDF fetches to every publisher/repository/lab domain
tried (AIP Publishing, INL, DOE, KTH DiVA) were still blocked, the same
network-policy limitation as before; findings below still come from the search
tool's own quoted extracts, the same [search-quoted] confidence tier, not from
reading a primary source end to end. Two real problems were found and are
flagged in the tables below rather than silently corrected: a wrong author
name on this project's single most load-bearing citation, and a PCM cost
citation that turned out to be for the wrong material class entirely (a
low-temperature building PCM review, not a high-temperature industrial one) --
exactly the kind of error a from-primary-source audit is supposed to catch, and
exactly why one is worth doing before this project's technology-ranking
conclusion is presented as more than a screening result.
...
| Storage CAPEX | 22-30 $/kWh-th (two-tank, current commercial CSP) | Allen, K., von Backström, T.W., Joubert, E.C., Gauché, P. (2016), "Rock bed thermal storage: Concepts and costs," AIP Conference Proceedings 1734, 050003. **Author name corrected this session** -- the original citation here read "Albrecht, K.J.," which does not match this paper's actual author list; re-searched and confirmed via Semantic Scholar/ADS. The 22-30 $/kWh-th figure itself matched an independent re-search exactly, unchanged. [search-quoted] |
...
| Storage CAPEX | less than 15 $/kWh-th at scale (>1000 MWh-th); as low as 5-8 $/kWh-th achievable depending on design and containment cost | Allen, K., von Backström, T.W., Joubert, E.C., Gauché, P. (2016), AIP Conference Proceedings 1734, 050003, "Rock bed thermal storage: Concepts and costs." **Author name and exact figure both corrected this session**: previously cited as "Albrecht, K.J." (wrong -- see the molten-salt row above) and paraphrased as "commonly cited <10 $/kWh-th," which overstated the paper's own stated ceiling (<15, not <10); the 5-8 $/kWh-th figure this project's own config actually uses is the paper's own best-case number, unchanged. [search-quoted] |
...
| Storage power CAPEX ($/MW) | 6.47 $/kW discharge, 20.67 $/kW charge (firebrick TES, industrial process heat) | Wikoff, H.M., Garfield, D., Hwang, S., Mendez Ribo, M., Ruth, M., Reese, S.B. (2025), "Benchmarking thermal energy storage cost for industrial process heat," *Applied Energy* 402, 126873. **Upgraded this session from pure [assumption] to a real, directly-on-topic citation** (the same industrial-process-heat application this project screens for), found via a dedicated search this parameter previously did not get. Firebrick is a different sensible-solid medium than crushed rock, so this is a same-category proxy, not a rock-bed-specific figure -- but it corroborates the config's own existing 20,000 EUR/MW [assumption] closely (the charge-power figure above converts to ~19-23 $/kW at typical EUR/USD rates), a reassuring cross-check rather than a reason to change the number. Distinct from the blower's own *operating* (electricity) cost, which `blower_specific_power_mw_per_mw` below separately prices -- capital and operating cost of the same equipment, not a double-count of one. |
...
| Storage CAPEX | **Under review -- see the correction note below.** This project's own config still uses 80,000 EUR/MWh-th (~80-90 $/kWh-th), the top of the *original*, now-flagged-wrong range. | **The original citation here was wrong, not just imprecise: re-checked this session and found to be citing the wrong material class.** Hirschey, J.R., Kumar, N., Turnaoglu, T., Gluesenkamp, K.R., Graham, S. (2021), "Review of Low-Cost Organic and Inorganic Phase Change Materials with Phase Change Temperature between 0 C and 65 C," 6th International High Performance Buildings Conference, Purdue (OSTI 1813229) -- as its own title states, this reviews *low-temperature* PCMs (salt hydrates, fatty acids, paraffin waxes) for *building* HVAC applications, 0-65 C. It says nothing about high-temperature nitrate PCM for industrial process heat, the material and application this project actually screens (melting ~306 C). The 50-120 $/kWh-th range attributed to it was not reproducible from this paper and should not have been used. A corrected search this session, targeted at high-temperature nitrate-salt/CSP PCM specifically, found encapsulated-PCM (EPCM) system costs of ~15-21 $/kWh-th -- $17.98/kWh(t) for one EPCM-TES system design; a DOE/Terrafore CSP EPCM program targeting the SunShot $15/kWh(t) goal, reporting "almost 40% or more" cost reduction vs. conventional two-tank sensible storage (this project's own 22-30 $/kWh-th Allen et al. figure above, so ~40% below that midpoint lands close to the same ~15-18 range independently); and a cascaded-EPCM study reporting ~21 $/kWh(t) at a 350 C temperature difference, the closest match to this project's own ~300 C process/bed temperature span. **This project's own PCM config (80,000 EUR/MWh-th) was set against the wrong, now-corrected citation and sits well above every figure the corrected search found** -- a real, actionable finding: PCM's "priced out entirely at every tested duration" result (Phase C3, Phase D.3) rests partly on a capex assumption now known to be too high, not confirmed to be too high by how much. `scripts/run_pcm_capex_sensitivity_experiment.py` tests this directly rather than silently changing the number every other committed result in this repository was generated against, sweeping `storage_capex_eur_per_mwh` from 10,000 to 80,000 EUR/MWh-th (spanning below the corrected literature floor up through the original config value) at the project's own headline case (300 C, flat load, tau=6h). **Result: the ranking finding survives.** PCM does not beat packed bed's already-committed reference cost (`outputs/phase_c_full_matrix/run_manifest.json`) anywhere in the swept range -- not even at 10,000 EUR/MWh-th, below the bottom of the corrected literature range (15,000-21,000 EUR/MWh-th). At the corrected range's own floor (15,000 EUR/MWh-th), PCM totals 4,034,052 EUR against packed bed's 4,004,310 EUR: closer than the original 80,000 EUR/MWh-th figure made it look, but still not competitive. `e_cap_mwh` at the LP's own optimum also *shrinks* as capex rises (45.93 MWh at 10,000 EUR/MWh-th vs 29.10 at 60,000, falling to ~0 at 80,000), the LP trading energy capacity for power capacity as storage gets more expensive per unit energy -- so the wrong citation did inflate the *margin* by which PCM lost, but not the qualitative finding itself. See `outputs/pcm_capex_sensitivity/run_manifest.json` and docs/RESULTS.md. |
...
## Economics: storage lifetime (shared assumption, all three technologies)

| Parameter | Value | Source |
|---|---|---|
| Storage lifetime | 25 years (`economics.storage_lifetime_years`, all five configs) | [assumption], now cross-checked this session (audit pass). NREL's own failure-analysis literature states the *design target* for molten-salt CSP tanks is a service life "of more than 30 years" -- so this project's 25-year figure is reasonably conservative against that target, not optimistic. Sanou, D. et al. / NREL, "Failure Analysis for Molten Salt Thermal Energy Storage Tanks for In-Service CSP Plants" (OSTI 2331241, 2024/2025). [search-quoted] |
| Real-world caveat, not currently modeled | Commercial two-tank molten-salt systems have repeatedly failed far short of either figure -- the same NREL report estimates a service life below 3 years under typical operating conditions once initial residual welding stress and floor deformation are accounted for, with creep and stress-relaxation cracking as the dominant mechanisms; the Crescent Dunes CSP plant's hot-tank leaks (four major leaks since 2015 commissioning) are the reported real-world instance, contributing to that project's eventual shutdown. | Same source. [search-quoted] |

This is a genuine, currently-undocumented risk this project does not model: `economics.storage_lifetime_years` feeds only the annualised-CAPEX calculation (a fixed number of years to amortise storage capex over), with no premature-failure probability, no design-quality distinction between technologies, and the same 25-year figure applied uniformly to molten salt, packed bed, and PCM alike -- even though the failure literature above is specific to two-tank molten-salt design (welded-floor thermal stress), not to packed-bed or PCM systems, which have different (and less-studied, in this audit pass) failure modes. A technology whose real-world tanks fail at 3 years instead of lasting 25 has its annualised storage cost understated roughly 8x by this project's economics model -- a much larger effect than any of the CAPEX-per-kWh corrections above, and one this screening tool does not currently sensitivity-test.
...
fetch, gated behind an `ENTSOE_API_KEY` environment variable that was not set for
most of this project's life (following the exact deferral pattern PyNEXUS itself
uses for the same reason: `data/entsoe.py` there is likewise built and unit-tested
but has never been run against a live credential). Until a key was available, Phase
...
**Update (2026-09-07): a real ENTSO-E credential was tested this session, and a
genuine (different) blocker was found.** `electricity_price.py`'s own fetch code
path worked as designed -- the `entsoe-py` client built the request correctly and
issued a well-formed HTTPS call. It did not reach ENTSO-E: this specific sandboxed
working environment's own outbound network egress policy blocks
`web-api.tp.entsoe.eu` at the proxy level (403 on the CONNECT, confirmed via the
proxy's own status endpoint), independent of the credential's validity or this
project's code. Per that proxy's own stated policy, this was not retried or routed
around. `scripts/fetch_entsoe_headline_prices.py` is written and ready (defaults:
SE_3 bidding zone -- Stockholm price area, this project's own KTH context -- one
full calendar year, 2024) but has not actually been run: it needs either an
environment whose egress policy allows that host, or the fetch run externally with
the resulting CSV handed back in. Every result in this repository remains
synthetic-price-only until that happens; nothing here should be read as claiming
real-price validation was completed.
